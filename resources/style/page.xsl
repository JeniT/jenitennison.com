  ...
  <xsl:variable name="uri" select="$metadata/rdf:Description[1]/@about" />
	<html>
		<head>
			...
			<xsl:call-template name="get-metadata">
				<xsl:with-param name="what" select="'meta'" />
				<xsl:with-param name="about" select="$uri" />
			</xsl:call-template>
			<xsl:call-template name="get-metadata">
...
<xsl:template match="html:meta" mode="get-metadata">
	<meta>
		<xsl:copy-of select="@*" />
	</meta>
</xsl:template>
	...
	<div id="colophon">
		...
		<p>
			...
			<xsl:choose>
  			...
  			<xsl:otherwise>
  			  <a href="mailto:jeni@jenitennison.com" class="mailto">Jeni Tennison</a>
  			</xsl:otherwise>
  		</xsl:choose>
		  <xsl:text>; generated using </xsl:text>
		  <xsl:call-template name="link">
		    <xsl:with-param name="link" select="system-property('xsl:vendor-url')" />
		    <xsl:with-param name="value" select="system-property('xsl:vendor')" />
		  </xsl:call-template>
